data = `.7F77F-J-F.LF-|-LL.J-|-F.7-|77L-L.FLFFJ7-7777-|7-F7.LFF--7F.LJ7-J7F-7-FL-7J77|-L-7-F-F--.J7..7FFLJ7..|7.F|--7FL|-|7F--J---JF-LL-L7-|.|-LJ7.F
...
solve = (data) => {
  ...
  x = arr.findIndex((x) => x.includes("S"));
  y = arr[x].indexOf("S");
  ...
  allDirs = Object.keys(dirs).map((x) => dirs[x]);

  move = ([px, py], [dx, dy]) => [px + dx, py + dy];

  curr = start;

  possibleDirs = allDirs;
  currDir = possibleDirs.find((dir) => {
    let [x, y] = move(curr, dir.d);
    try {
      return dir.vals.indexOf(arr[x][y]) !== -1;
    } catch (e) {
      return false;
    }
  });
  ...
  path = [curr];
  while (!flag) {
    curr = move(curr, currDir.d);
    path.push(curr);
    len++;
    let [x, y] = curr;
    let ch = arr[x][y];
    if (ch == "S") flag = true;
    else currDir = dirs[currDir.next(ch)];
    c++;
  }
  ...
  res = arr.map((row, i) => {
    return row.map((ch, j) => {
      if (ch != ".") return "*";
      let prev = row.slice(0, j);
      let up = arr.map((r) => r[j]).slice(0, i);
      hBars = prev.filter((x) => x == "|").length;
      vBars = up.filter((x) => x == "-").length;
      // console.log(ch, j, prev);
      // return [ch, hBars, vBars];

      if (ch == "." && hBars % 2 != 0 && vBars % 2 != 0) return 1;
      else return 0;
    });
  });
  ...
  A =
    Math.abs(
      path
        .slice(1)
        .map((p, i) => det(path[i], p))
        .reduce((a, b) => a + b, 0),
    ) / 2;

  // Pick's theorem
  return A + 1 - (path.length - 1) / 2;
};

det = (a, b) => a[0] * b[1] - b[0] * a[1];

console.log("sample 1 should be", 4, "is", solve(sample1));
console.log("sample 1 should be", 8, "is", solve(sample2));
